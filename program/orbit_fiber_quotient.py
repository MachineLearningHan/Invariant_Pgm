# PAPER: Prop 3.1-3.2, Figure 2 (orbit/fiber/quotient). See RESULTS_TO_CODE.md
"""
...
print(f"input: 30 clones of A + 30 clones of B (A,B share h, differ in reader)", flush=True)
print(f"representation quotient (gram) -> {len(repq)} class ; "
      f"A and B collapsed together: {any(len(v) > 1 for v in repq.values())}", flush=True)
print(f"joint quotient (output func)   -> {len(joint)} classes; "
      f"every class pure (one program): {all(len(v) == 1 for v in joint.values())}", flush=True)
print("\n=> capability lives on the JOINT quotient; the representation quotient "
      "(what CKA sees) is coarser and cannot separate A from B.", flush=True)
